...
public:
    ...
    diffuse_light(const texture* a, bool two_sided = false) : emit(a), two_sided(two_sided) {}
...
public:
    directional_light(mem_arena &arena, const colour &c, double inner_width, double outer_width) :
        emit(arena.alloc<solid_colour>(c)), cos_inner(cos(inner_width)), cos_outer(cos(outer_width)) {}
    directional_light(const texture* a, double inner_width, double outer_width) :
        emit(a), cos_inner(cos(inner_width)), cos_outer(cos(outer_width)) {}
    ...
    float falloff(const vec3& normal, const vec3& direction) const {
        auto cos_theta = normal.dot(direction);
    ...
    virtual colour emitted(const ray &r_in, const hit_record &rec) const override {
        if (!rec.front_face)
            return colour(0);
        auto I = emit->value(rec.u, rec.v, rec.p);
        auto unit_direction = unit_vector(r_in.direction());
        return I * falloff(rec.normal, -unit_direction);
    }

private:
    const texture* emit;
    double cos_inner, cos_outer;
};
