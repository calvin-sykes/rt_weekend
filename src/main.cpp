...
int main(int argc, char **argv) {
    ...
    mem_arena arena;
    scene scn = get_scene(arena, SCENE_PHONG,
    ...
    rendrr.render(scn);
    rendrr.dump(filetype, filename.c_str(), expose);

    /*auto idx = filename.find(".");
    std::string filename_hdr = filename.replace(idx+1, 3, "hdr");
    rendrr.dump(FILE_TYPE_HDR, filename_hdr.c_str(), expose);*/

    // auto idx = filename.find(".");
    // if (idx != std::string::npos) {
    //     std::string variance_filename("variance_" + filename.replace(idx+1, 3, "txt"));
    //     rendrr.dump_variance(FILE_TYPE_TEXT, variance_filename.c_str());
    // }
