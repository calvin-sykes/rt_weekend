    ...
    for (int i = 0; i < depth; i++) {
        ...
        if (srec.pure_specular) {
            ...
            if constexpr(use_roulette) {
                //if (i > 1) { // Force at least one scattering
                //    // Russian Roulette
                //    // Randomly terminate a path with a probability inversely proportional to the throughput
                //    //auto p = std::max(max_axis(throughput), 0.1f);
                //    auto p = max_axis(throughput);
                //    if (random_number() > p)
                //        break;
                //    
                //    // Add back the energy lost by randomly terminating paths
                //    //throughput *= (1.0f / std::max(p, 0.1f));
                //    throughput *= 1.0f / p;
                //}
                 if (i > 1) { // Force at least one scattering
                     // Russian Roulette
                     // Randomly terminate a path with a probability inversely proportional to the throughput
                     auto q = std::max(0.05f, 1.0f - max_axis(throughput));
                     if (random_number() < q)
                         break;
    ...
    #pragma omp parallel num_threads(n_threads)
    {
        ...
        for (int j = 0; j < h; j++) {
            for (int i = 0; i < w; i++) {
                ...
                online_stats<colour> stddev_lum;
                for (int s = 0; s < num_samples; s++) {
                    ray r = cam.get_ray(i, j);
                    auto pix_colour = ray_colour(arena, r, background, world, lights, max_depth);
                    stddev.update(pix_colour);
                    stddev_lum.update(colour(luminance(pix_colour)));
                ...
                if (tid == 0 && (count - reported_count >= step_size)) {
                    ...
                    std::cerr << "\rTraced " << count * num_samples << '/' << static_cast<long long>(total_size) * num_samples << " rays ("
...
void renderer::dump(FILE_TYPE ftype, const char *filename, float exposure) const {
    auto resampled_buffer = resample();
    
    if (ftype != FILE_TYPE_HDR) {
        tone_mapping(resampled_buffer, exposure);
    }
