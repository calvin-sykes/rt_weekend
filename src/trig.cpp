...
double my_exp(double x) {
    ...
    constexpr double c1 =  1.77187919e-04;
    constexpr double c2 =  3.03212820e-01;
    constexpr double c3 = -2.24169036e-01;
    constexpr double c4 = -7.90302044e-02;

	constexpr double a = 4503599627370496.0; // (1 << 53)
	constexpr int64_t b = 4607182418800017408; // 1023 * (1 << 53)

    x *= log2e;
	double fx = x - floor(x);

    double K = c1 + fx * (c2 + fx * (c3 + fx * c4));
    x -= K;
